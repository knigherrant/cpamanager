        ...
        if(!$this->value){
            if($this->form->getField('storage_type')->value == 'remote'){
                ...
                if($this->form->getField('storage_path_file')->value){
                    $fileType = SignatureDocHelper::getFileType($this->form->getField('storage_path_file')->value);
            ...
            $image = JUri::root().'administrator/components/com_cpamanager/assets/images/nothumbnail.png';
        }else{
            $image = JUri::root().SignatureDocHelper::getDirPath('thumbs').'/'.$this->value;
        }
        ob_start();
        ?>
        <div class="image-picker thumbnail-picker" style="margin-top: 5px">
            ...
            <div id="choose-thumbnail" class="dropdown btn-group" style="float:left;margin-left:5px">
                <button class="btn choose-photo-button dropdown-toggle" id="profile_header_upload" type="button" data-toggle="dropdown" style="float:none">
                    <?php echo JText::_('COM_SIGNATUREDOC_CHANGE_THUMBNAIL')?>
                ...
                <ul class="dropdown-menu">
                    <li id="thumbnail-choose-existing" class="dropdown-link">
                        <div id="image-selector" class="image-selector">
                            ...
                            <a class="modal modal-button btn" rel="{'handler': 'iframe', 'size': {'x': 690}}" href="index.php?option=com_cpamanager&view=files&layout=select&tmpl=component&ctn=thumbs&ctn_type=image&field=<?php echo $this->id;?>"><?php echo JText::_('COM_SIGNATUREDOC_CHOOSE_EXISTING_IMAGE')?></a>
                        </div>
                    </li>
                    <li class="divider"></li>
                    <li id="thumbnail-delete-image" class="pretty-link dropdown-link">
                        <a href="javascript:void(0)" class="dropdown-link btn" id="image-selector-clear"><?php echo JText::_('COM_SIGNATUREDOC_CLEAR')?></a>
                    </li>
                </ul>
